...
AxonFlow 当前存在一个关键基础设施缺失：Agent 的 tool calling 循环未接通。
...
文件：`src/axonflow/llm/gateway.py`
...
文件：`src/axonflow/config/models.py`
...
文件：`src/axonflow/config/loader.py`
...
文件：`src/axonflow/llm/prompt_builder.py`
...
文件：`src/axonflow/observability/execution_log.py`（新建）
...
- `AxonFlowEngine` 初始化时创建 `ExecutionLogger` 实例
...
| 文件 | 变更 |
|------|------|
| `src/axonflow/llm/gateway.py` | LLMResponse 增加 tool_calls；chat() 解析 tool_calls |
| `src/axonflow/core/agent.py` | handle_message() 补全多轮工具调用循环；集成 ExecutionLogger |
| `src/axonflow/tools/base.py` | ToolRegistry 增加 execute() 调度方法 |
| `src/axonflow/config/models.py` | AgentConfig 增加 skills 字段 |
| `src/axonflow/config/loader.py` | 新增 load_skill_content()、_resolve_script_refs() |
| `src/axonflow/llm/prompt_builder.py` | system prompt 注入 skill 内容 |
| `src/axonflow/engine.py` | 初始化 ExecutionLogger，传递给 Agent |

### 新建文件

| 文件 | 用途 |
|------|------|
| `src/axonflow/observability/execution_log.py` | ExecutionLogEntry + ExecutionLogger |
